...
def get_unet(img_rows, img_cols, flt=64, pool_size=(2, 2, 2), init_lr=1.0e-5):
    ...
    conv2 = Conv2D(flt*2, (3, 3), activation='gelu', padding='same')(pool1)
    conv2 = Conv2D(flt*2, (3, 3), activation='gelu', padding='same')(conv2)
    ...
    conv4 = Conv2D(flt*8, (3, 3), activation='gelu', padding='same')(pool3)
    conv4 = Conv2D(flt*8, (3, 3), activation='gelu', padding='same')(conv4)
    pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)

    conv5 = Conv2D(flt*16, (3, 3), activation='gelu', padding='same')(pool4)
    conv5 = Conv2D(flt*8, (3, 3), activation='gelu', padding='same')(conv5)

    up6 = concatenate([Conv2DTranspose(flt*8, (2, 2), strides=(2, 2), padding='same')(conv5), conv4], axis=3)
    conv6 = Conv2D(flt*8, (3, 3), activation='gelu', padding='same')(up6)
    conv6 = Conv2D(flt*4, (3, 3), activation='gelu', padding='same')(conv6)

    up7 = concatenate([Conv2DTranspose(flt*4, (2, 2), strides=(2, 2), padding='same')(conv6), conv3], axis=3)
    conv7 = Conv2D(flt*4, (3, 3), activation='gelu', padding='same')(up7)
    conv7 = Conv2D(flt*2, (3, 3), activation='gelu', padding='same')(conv7)

    up8 = concatenate([Conv2DTranspose(flt*2, (2, 2), strides=(2, 2), padding='same')(conv7), conv2], axis=3)
    conv8 = Conv2D(flt*2, (3, 3), activation='gelu', padding='same')(up8)
    conv8 = Conv2D(flt, (3, 3), activation='gelu', padding='same')(conv8)

    up9 = concatenate([Conv2DTranspose(flt, (2, 2), strides=(2, 2), padding='same')(conv8), conv1], axis=3)
    conv9 = Conv2D(flt, (3, 3), activation='gelu', padding='same')(up9)
    conv9 = Conv2D(flt, (3, 3), activation='gelu', padding='same')(conv9)
